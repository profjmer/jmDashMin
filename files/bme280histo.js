...
function setupWebsocket() {
	...
	ws.onopen = function(e) {
		console.log("Client en ligne,requête bme histo");
		...
		if (typeof e.data === "string") {
			...
			try {
				...
				if(jsonObject.type=="BME280"){
					...
					lineChartData.datasets[2].data.push({x:temps,y:pression}); 					
					window.myLine.update();
					
				}

				if(jsonObject.type=="BME280Histo"){
					//log("Message received: " + e.data);
					// Extraire les valeurs des clés
					var temps = jsonObject.ts;
					var temp = jsonObject.T;				
					var humidity = jsonObject.HR;
					var pression = jsonObject.P;		
					...
					for(i=0;i<temps.length;i++){
							...
							lineChartData.datasets[2].data[i]={"x":temps[i],"y":pression[i]};
					}						
					window.myLine.update();			
				}
